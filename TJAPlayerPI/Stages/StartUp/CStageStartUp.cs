    ...
    public override int On進行描画()
    {
        if (!base.b活性化してない)
        {
            if (base.b初めての進行描画)
            {
                ...
                this.list進行文字列.Add("Release: " + TJAPlayerPI.VERSION);
            ...
            lock (this.list進行文字列)
            {
                ...
                {
                    TJAPlayerPI.app.act文字コンソール.tPrint(x, y, C文字コンソール.EFontType.白, str);
                    y += 24;
                }
                TJAPlayerPI.app.act文字コンソール.tPrint(x, y, C文字コンソール.EFontType.白, this.str現在進行中);
            }
            //-----------------
            #endregion
