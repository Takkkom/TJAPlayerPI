...
internal class CActStageFailed : CActivity
{
    ...
    public void Start()
    {
        this.dbFailedTime = TJAPlayerPI.app.Timer.n現在時刻ms;
        this.ct進行 = new CCounter(0, 1000, 2, TJAPlayerPI.app.Timer);
        if (TJAPlayerPI.app.ConfigToml.PlayOption._GameMode != EGame.OFF)
        {
            this.ct進行 = new CCounter(0, 4000, 2, TJAPlayerPI.app.Timer);
        }
    }
    ...
    public override int On進行描画()
    {
        ...
        if (TJAPlayerPI.app.ConfigToml.PlayOption._GameMode == EGame.完走叩ききりまショー || TJAPlayerPI.app.ConfigToml.PlayOption._GameMode == EGame.完走叩ききりまショー激辛)
        {
            ...
            {
                if (TJAPlayerPI.app.Tx.Failed_Game is not null)
                    TJAPlayerPI.app.Tx.Failed_Game.t2D描画(TJAPlayerPI.app.Device, 0, 0);

                int num = (TJAPlayerPI.DTX[0].listChip.Count > 0) ? TJAPlayerPI.DTX[0].listChip[TJAPlayerPI.DTX[0].listChip.Count - 1].n発声時刻ms : 0;
        ...
        else
        {
            if (this.ct進行.n現在の値 < 100)
            {
                ...
                if ((x != 640) && (TJAPlayerPI.app.Tx.Failed_Stage is not null))
                {
                    TJAPlayerPI.app.Tx.Failed_Stage.t2D描画(TJAPlayerPI.app.Device, 0, 0, new Rectangle(x, 0, 640 - x, 720));
                    TJAPlayerPI.app.Tx.Failed_Stage.t2D描画(TJAPlayerPI.app.Device, 640 + x, 0, new Rectangle(640, 0, 640 - x, 720));
                }
            }
            else
            {
                if (TJAPlayerPI.app.Tx.Failed_Stage is not null)
                {
                    TJAPlayerPI.app.Tx.Failed_Stage.t2D描画(TJAPlayerPI.app.Device, 0, 0);
                }
                if (this.ct進行.n現在の値 <= 250)
                {
                    ...
                    if (TJAPlayerPI.app.Tx.Failed_Stage is not null)
                    {
                        TJAPlayerPI.app.Tx.Failed_Stage.t2D描画(TJAPlayerPI.app.Device, num2, y);
                    }
                }
                if (!this.b効果音再生済み)
                {
                    TJAPlayerPI.app.Skin.SystemSounds[Eシステムサウンド.SOUNDステージ失敗音].t再生する();
                    this.b効果音再生済み = true;
                }
            }
        }
    ...
    private void t文字表示(int x, int y, string str)
    {
        ...
        if (TJAPlayerPI.app.Tx.Balloon_Number_Roll is null)
            return;
        ...
        foreach (char ch in str)
        {
            if (this.st文字位置.TryGetValue(ch, out var pt))
            {
                ...
                if (ch == '%')
                    rectangle.Width = 80;
                TJAPlayerPI.app.Tx.Balloon_Number_Roll.t2D描画(TJAPlayerPI.app.Device, x - (62 * str.Length / 2), y, rectangle);
            }
            x += 62;
        }
    }
