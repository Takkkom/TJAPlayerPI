...
using static TJAPlayerPI.PlayerLane;
...
internal class TaikoLaneFlash : CActivity
{
    ...
    {
        PlayerLane = new PlayerLane[TJAPlayerPI.app.ConfigToml.PlayOption.PlayerCount];
        ...
        for (int i = 0; i < (int)FlashType.Total; i++)
        {
            switch (i)
            {
                case (int)FlashType.Red:
                    Flash[i] = new LaneFlash(ref TJAPlayerPI.app.Tx.Lane_Red, player);
                    break;
                case (int)FlashType.Blue:
                    Flash[i] = new LaneFlash(ref TJAPlayerPI.app.Tx.Lane_Blue, player);
                    break;
                case (int)FlashType.Hit:
                    Flash[i] = new LaneFlash(ref TJAPlayerPI.app.Tx.Lane_Yellow, player);
                    break;
                default:
                    break;
            }
