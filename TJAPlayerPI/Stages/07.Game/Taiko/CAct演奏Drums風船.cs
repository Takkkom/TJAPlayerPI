            ...
            for (int j = 0; j < 5; j++)
            {
                if (n残り打数[j] < n連打数)
                {
                    if (TJAPlayerPI.app.Tx.Balloon_Breaking[j] is not null)
                        TJAPlayerPI.app.Tx.Balloon_Breaking[j].t2D描画(TJAPlayerPI.app.Device, TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.BalloonX[nPlayer] + (this.ct風船ふきだしアニメ.n現在の値 == 1 ? 3 : 0) + TJAPlayerPI.app.Skin.SkinConfig.Game.ScrollFieldX[nPlayer] - 414, TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.BalloonY[nPlayer]);
                    break;
                }
            }
            //1P:31 2P:329
