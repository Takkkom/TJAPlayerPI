...
internal class CActRunner : CActivity
{
    ...
    {
        if (TJAPlayerPI.app.Tx.Runner is null)
            return;
        ...
        if (pChip.nチャンネル番号 < 0x15 || (pChip.nチャンネル番号 >= 0x1A))
        {
            for (int i = 0; i < 128; i++)
            {
                if (!stRunners[i].b使用中)
                {
                    ...
                    stRunners[i].ct進行 = new CCounter(0, TJAPlayerPI.app.LogicalSize.Width, TJAPlayerPI.app.Skin.SkinConfig.Game.Runner.Timer, TJAPlayerPI.app.Timer);
    ...
    public override int On進行描画()
    {
        for (int i = 0; i < 128; i++)
        {
            if (stRunners[i].b使用中)
            {
                ...
                if (stRunners[i].ct進行.b終了値に達した || stRunners[i].fX > TJAPlayerPI.app.LogicalSize.Width)
                {
                ...
                TJAPlayerPI.app.Tx.Runner?.t2D描画(TJAPlayerPI.app.Device, (int)(StartPoint_X[stRunners[i].nPlayer] + stRunners[i].fX), StartPoint_Y[stRunners[i].nPlayer], new Rectangle(stRunners[i].nNowPtn * Size[0], stRunners[i].nType * Size[1], Size[0], Size[1]));
            }
        }
        return base.On進行描画();
    }
