...
internal class CActRoll : CActivity
{
        ...
        for (int i = 0; i < TJAPlayerPI.app.ConfigToml.PlayOption.PlayerCount; i++)
        {
            ...
            if (this.ct連打枠カウンター[player].b終了値に達してない)
            {
                if (ct連打枠カウンター[player].n現在の値 > 1333 && !FadeOutCounter[player].b進行中)
                {
                    ...
                    FadeOutCounter[player].t開始(0, fadenum - 1, 1, TJAPlayerPI.app.Timer);
                }
                var opacity = (fadenum - FadeOutCounter[player].n現在の値) * 255 / fadenum;
                if (TJAPlayerPI.app.Tx.Balloon_Roll is not null)
                    TJAPlayerPI.app.Tx.Balloon_Roll.Opacity = opacity;
                if (TJAPlayerPI.app.Tx.Balloon_Number_Roll is not null)
                    TJAPlayerPI.app.Tx.Balloon_Number_Roll.Opacity = opacity;


                if (TJAPlayerPI.app.Tx.Balloon_Roll is not null)
                    TJAPlayerPI.app.Tx.Balloon_Roll.t2D描画(TJAPlayerPI.app.Device, TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.RollFrameX[player], TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.RollFrameY[player]);
                this.t文字表示(TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.RollNumberX[player], TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.RollNumberY[player], n連打数, player);
            }
        }
    ...
    {
        this.ct連打枠カウンター[player] = new CCounter(0, 1500, 1, TJAPlayerPI.app.Timer);
    ...
    private void t文字表示(int x, int y, int n連打, int nPlayer)
    {
        ...
        for (int index = n連打.ToString().Length - 1; index >= 0; index--)
        {
            ...
            if (TJAPlayerPI.app.Tx.Balloon_Number_Roll is not null)
            {
                TJAPlayerPI.app.Tx.Balloon_Number_Roll.vcScaling.X = TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.RollNumberScale;
                TJAPlayerPI.app.Tx.Balloon_Number_Roll.vcScaling.Y = TJAPlayerPI.app.Skin.SkinConfig.Game.Balloon.RollNumberScale + RollScale[this.ct連打アニメ[nPlayer].n現在の値];
